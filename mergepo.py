...
class POMergerEntry:
    ...
    def __init__(self, entry, is_matched=None, msgid_in_exported=None, is_duplicate=None):
        self.entry = entry
        self.is_matched = is_matched
        self.msgid_in_exported = msgid_in_exported
    ...
    @property
    def no_references(self):
        return self.excluded_occurrences >= set(self.entry.occurrences + self.added_occurrences)

    @property
    def included_occurrences(self):
        return [o for o in self.entry.occurrences + self.added_occurrences if o not in self.excluded_occurrences]
    ...
    def merge_occurrences(self, other, regex):
        ...
        excluded_occurrences = set(
            filter_occurrences(self.entry.occurrences + self.added_occurrences + list(self.excluded_occurrences), regex)
        )
        for occurrence in other.entry.occurrences:
    ...
    def match_occurrences(self, other, regex):
        ...
        :return:
        """
        included_occurrences = set(
            filter_occurrences([o for o in other.entry.occurrences + other.added_occurrences if o not in other.excluded_occurrences], regex)
        )
        for occurrence in self.entry.occurrences + self.added_occurrences:
            if occurrence not in included_occurrences:
    ...
    def matches_regex(self, regex, match_empty=True):
        ...
        :return:
        """
        return bool(filter_occurrences(self.entry.occurrences, regex)) or (match_empty and not self.entry.occurrences)
...
class POMerger:
    ...
    def correct_occurrences(self):
        for original_entry in self.original_entries:
            exported_entries = self.exported_entries_by_msgid.get(original_entry.entry.msgid, None)
            if original_entry.matches_regex(self.regex) and exported_entries:
                original_entry.match_occurrences(exported_entries[0], self.regex)
    ...
    def run(self):
        ...
        with open(self.output_path, 'w', encoding='utf-8') as output_file:
            ...
            for entry in self.original_entries:
                ...
                if duplicate_entry or not_in_exported or entry.no_references:
                    if duplicate_entry:
                        removal_reason = 'Duplicate entry'
                    elif not_in_exported:
                        removal_reason = 'Not in exported file'
                    elif entry.no_references:
                        removal_reason = 'No references'
                    add_entry = False
                    log.append((entry, 'red'))
                    removed_count += 1
                    self.lines_removed += str(entry).count('\n')

                if add_entry:
                    output_file.write(str(entry))
                    added_entries.add(entry)
                    lines_added, lines_removed = len(entry.added_occurrences), len(entry.excluded_occurrences)
                    if lines_added or lines_removed:
                        log.append((entry, 'cyan'))
                        merged_count += 1
                    else:
                        log.append((entry, ''))
            ...
            for entry in self.exported_entries:
                if entry.entry.msgid not in self.original_entries_by_msgid:
                    output_file.write(str(entry))
                    log.append((entry, 'green'))
                    added_count += 1
                    self.lines_added += str(entry).count('\n')
